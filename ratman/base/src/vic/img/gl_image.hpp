...
//======================================================================
//---HDR---//
#ifndef VIC_IMG_GL_IMAGE_HPP
#define VIC_IMG_GL_IMAGE_HPP
...
namespace vic {
  ...
  namespace img {
	...
	extent_[0] = other.extent_[0];
	extent_[1] = other.extent_[1];
	extent_[2] = other.extent_[2];
	extent_[3] = other.extent_[3];
	std::size_t other_size = other.size();
	data_ = other_size ? (new value_type[other_size]) : 0;
	for (std::size_t i=0; i<other_size; ++i) {
   ...
   public: // Alpha
    ...
    public: // Alpha
      ...
      /// reshape alpha channel between min_out and max_out
      void channel_rescale(std::size_t c, 
			   value_type min_out, 
			   value_type max_out) {
	assert(c<channels());
	std::size_t nc=channels()-1;
	const float zero = float(sl::scalar_math<value_type>::zero());
	const float one  = float(sl::scalar_math<value_type>::finite_upper_bound());
	value_type *ptr, *ptr_end = end();
	for (ptr=begin(); ptr<ptr_end; ptr+=nc+1) {
	  float v =  ((one-float(ptr[c]))*min_out+(float(ptr[c])-zero)*max_out)/(one-zero);
	  ptr[c] = static_cast<value_type>(sl::median(v,zero,one)+0.4);
	}
      }

      /// Set last channel to 0 if average of others is less than v, to max instead
      this_t& set_alpha_from_black(value_type v=10) {
	assert(channels()>1);
	std::size_t nc=channels()-1;

	value_type zero = sl::scalar_math<value_type>::zero();
	value_type one  = sl::scalar_math<value_type>::finite_upper_bound();
	...
	return *this;
      ...
      /// Set last channel to 0 if average of others is more than v, to max instead
