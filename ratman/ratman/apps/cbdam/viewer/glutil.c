...
typedef StrokeFontPtr GLUTIL_strokeFont;
...
static StrokeFontRec glutil_StrokeRoman = { "Roman", 128, chars, 119.048, -33.3333 };

static const  GLUTIL_strokeFont GLUTIL_STROKE_ROMAN = &glutil_StrokeRoman;

/////////////////////////////////////////////////////////////

static void glutil_printc(GLUTIL_strokeFont font, int c) {
  // Derived from glutil_, Copyright (c) Mark J. Kilgard, 1994.
  // FIXME: Rewrite
  const StrokeCharRec *ch;
  const StrokeRec *stroke;
  const CoordRec *coord;
...
void glutil_begin_screen_coordinates_system() {
  ...
  {
    /* Maximum contrast color by default */
    GLfloat C[4];
...
void glutil_end_screen_coordinates_system() {
  glMatrixMode(GL_PROJECTION); 
  glPopMatrix();
  glMatrixMode(GL_MODELVIEW);
...
static char* glutil_new_vmessage(const char *fmt,
                                 va_list ap) {
  int n, size = 100;
  char *result;
...

  {
	...
	if ((result = (char*)realloc (result, size)) == NULL) {
	  done=1; /* OUT OF MEMORY */
	}
      }
    }
...
void glutil_printf(const GLfloat x,
                   const GLfloat y,
                   const GLfloat h,
                   const char *fmt,
                   ...) {
  assert(fmt);
  {
    va_list ap;
    va_start(ap,fmt);
    glutil_vprintf(x,y,h,fmt,ap);
    va_end(ap);
  }
}

void glutil_printf_invert_y(const GLfloat x,
			    const GLfloat y,
			    const GLfloat h,
			    const char *fmt,
			    ...) {
...
void glutil_vprintf(const GLfloat x,
                    const GLfloat y,
                    const GLfloat h,
                    const char *fmt,
                    va_list ap) {
  static const GLfloat font_scale = 119.05f - 33.33f;

  //  glutil_begin_screen_coordinates_system();
  {
    GLfloat scale = h/font_scale;
    glPushMatrix();
    // Removed to keep valid external modelview matrix
    // glLoadIdentity();
    glTranslatef(x, y, 0.0);
    glScalef(scale, scale, scale);

    {
      char *msg = glutil_new_vmessage(fmt,ap);
      if (msg) {
	const char* c = msg;
	while(*c) {
	  glutil_printc(GLUTIL_STROKE_ROMAN, *c);
	  ++c;
	}
	free(msg);
      }
  ...
  //  glutil_end_screen_coordinates_system();
}
