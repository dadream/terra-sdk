...
namespace sl {
  ...
  public: // Creation & Destruction
    ...
    inline floating_cone(const vector_t& n,
                         const value_t& alpha = sl::scalar_math<value_t>::zero()): axis_(n), sin_angle_(std::sin(alpha)), cos_angle_(std::cos(alpha)) {
    }

    /// The from axis, cos angle, sin angle
    ...
    inline floating_cone(const point_t& e,
      ...
      if (b.is_empty()) {
        ...
        // Standard direction cone
        axis_ = b.center() - e;
        value_t a2 = axis_.two_norm_squared();
        value_t a  = std::sqrt(a2);
        cos_angle_ = std::sqrt(a2 - sqr(b.radius())) / a;
        sin_angle_ = b.radius() / a;
        axis_ /= a;
      }
    }
  ...
  public: // Assignment
    ...
    void grow(const vector_t& other) {
      if (is_empty()) {
        ...
        value_t result_angle = std::acos(sl::median(axis().dot(other),
                                                     -sl::scalar_math<value_t>::one(), 
                                                     sl::scalar_math<value_t>::one())); // O..Pi
        if (result_angle > angle()) {
          cos_angle_ = std::cos(result_angle);
   ...
   /// is the object currently building a bounding volume?
    ...
    virtual inline void end_model() {
      ...
      if (!vectors_.empty()) {
        ...
        mbp.end_model();
        vector_t axis = mbp.last_bounding_volume().center().as_vector().ok_normalized();
#else
        vector_t axis;
        for (std::size_t i=0; i<vectors_.size(); ++i) {
          axis += vectors_[i];
        }
        axis = axis.ok_normalized();
#endif
