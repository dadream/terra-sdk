...
//======================================================================
//---HDR---//
#ifndef SL_FIXED_SIZE_LINE_SEGMENT_HPP
#define SL_FIXED_SIZE_LINE_SEGMENT_HPP
...
namespace sl {
  ...
  /**
   *  Line segments of fixed dimension: these are defined as
   *  R(t) = O + t*D, where 0<= t <=1 and D is not necessarily a unit
   *  vector
   */
  template <size_t DIMENSION, class T>
  class fixed_size_line_segment {
  public: // Constants and types

    enum { dimension = DIMENSION, hdimension = DIMENSION+1 };

    typedef fixed_size_line_segment<DIMENSION,T>  this_t;
    typedef T       value_t;
  ...
  public: // Creation, Copy & Destruction
    ...
    /// Default init (zero)
    inline fixed_size_line_segment() {
    }

    /// Fast init (garbage), handle with care!
    inline fixed_size_line_segment(tags::not_initialized tag): origin_(tag), direction_(tag) {
      // Garbage in, use with care
    }

    /// Init from origin and direction
    inline explicit fixed_size_line_segment(const point_t& o,
				   const vector_t& d)
  ...
  public: // Access
    ...
    static inline value_t tmax() {
      return scalar_math<value_t>::one();
    }
    ...
    /// The extremity of the line segment
    inline point_t extremity() const {
...
template <size_t DIMENSION, class T>
std::ostream& operator <<(std::ostream& s, const sl::fixed_size_line_segment<DIMENSION,T>& a) {
  s << a.origin() << std::endl;
...
template <size_t DIMENSION, class T>
std::istream& operator >>(std::istream& s, sl::fixed_size_line_segment<DIMENSION,T>& a) {
  s >> a.origin();
  s >> a.direction();
  return s;
}
...
inline sl::fixed_size_line_segment<N_dimension,T> transformation(const sl::linear_map_base<T_tag, N_dimension, T>& lm, 
							const sl::fixed_size_line_segment<N_dimension,T>& x) {
  return sl::fixed_size_line_segment<N_dimension,T>(transformation(lm,x.origin()),
					   transformation(lm,x.direction()));
}

/// the line x transformed by the inverse of map lm
template <class T_tag, size_t N_dimension, class T>
inline sl::fixed_size_line_segment<N_dimension,T> inverse_transformation(const sl::linear_map_base<T_tag, N_dimension, T>& lm, 
								const sl::fixed_size_line_segment<N_dimension,T>& x) {
  return sl::fixed_size_line_segment<N_dimension,T>(inverse_transformation(lm,x.origin()),
					   inverse_transformation(lm,x.direction()));
}

/// the line x transformed by map lm
template <class T_tag, size_t N_dimension, class T>
inline sl::fixed_size_line_segment<N_dimension,T> operator * (const sl::linear_map_base<T_tag, N_dimension, T>& lm, 
						     const sl::fixed_size_line_segment<N_dimension,T>& x) {
...
namespace sl {
  ...
  typedef fixed_size_line_segment<2,float> line_segment2f;
  /// 3D line with single precision floating line components
  typedef fixed_size_line_segment<3,float> line_segment3f;
  /// 4D line with single precision floating line components
  typedef fixed_size_line_segment<4,float> line_segment4f;

  /// 2D line with double precision floating line components
  typedef fixed_size_line_segment<2,double> line_segment2d;
  /// 3D line with double precision floating line components
  typedef fixed_size_line_segment<3,double> line_segment3d;
  /// 4D line with double precision floating line components
  typedef fixed_size_line_segment<4,double> line_segment4d;
}
