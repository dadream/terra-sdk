...
namespace sl {
  ...
  static inline double pow2quantized_xform_error(const xform_t& xform,
    ...
    int32_matrix_t Mw= M, QMw= tags::not_initialized(), M_prime= tags::not_initialized();
    xform.forward(Mw);
  ...
  public:
    ...
    static inline int32_t average(const int_matrix_t& M) {
      double M_sum = 0.0;
      uint32_t isz = M.extent()[0];
      uint32_t jsz = M.extent()[1];
    ...
    static inline void compress(entropy_codec_t& codec,
      ...
      if (sz>=isz && sz>=jsz) {
        ...
        int32_matrix_t  MM= tags::not_initialized();
	int32_t         MM_amax = 0;
        for (uint32_t i=0; i<sz; ++i) {
    ...
    static inline void compress(entropy_codec_t& codec,
                                ...
                                uint32_t log2_threshold) {
      int32_matrix_t Mw=M, QMw= tags::not_initialized();
      matrix_transform_t::forward(Mw);
      rescaled_pow2quantize_in(QMw, Mw, log2_threshold);
    ...
    static inline void decompress(entropy_codec_t& codec,
                                  int32_matrix_t& M,
                                  uint32_t log2_threshold) {
      int32_matrix_t QMw= tags::not_initialized();
...
namespace sl {
  ...
  void hidwt_array_codec::int_quantized_compress(const dense_array<int_t,2,void>& array,
    ...
    // Encode array shape
    const uint32_t h = array.extent()[0];
    const uint32_t w = array.extent()[1];
  ...
  void hidwt_array_codec::int_compress(const dense_array<int_t,2,void>& array,
    ...
    const uint32_t max_buffer_size = 1024+32*array.count();
