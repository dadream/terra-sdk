...
namespace sl {
  ...
  public:
    ...
    inline const subvalue_t &operator[](std::size_t i) const {
      assert(i<dimension);
      return c_[i];
    }
  ...
  public: // KNN
	...
	float_t sqr_bot = (knn_maxradius>=sqrt_maxd2_)?(maxd2_):(bot*bot);
  ...
  protected:
	...
	for (std::size_t d=0; d<dimension; ++d) {
	  const float_t delta = (float_t(node_value[d]) - 
				 float_t(p[d]));
	  dist2 += delta * delta;
	}
