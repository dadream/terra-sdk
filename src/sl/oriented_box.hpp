...
namespace sl {
  ...
  void oriented_box_builder<2,T>::construct() {
	...
	SL_TRACE_OUT(1) << "ROTATING CALIPER: AREA/4 = " << min_area_div_4 << " DONE = " << done << std::endl;
      ...
      //////////////////////////////// HUGE HACK
      {
	   this->construct_from_basis((this->last_bounding_volume_).from_box_space_map());	if ((sl::abs(hsx-(this->last_bounding_volume_).half_side_lengths()[0]) > 100.0 * sl::epsilon(value_t()) + 0.01 * sl::max(sl::abs(hsx), sl::abs((this->last_bounding_volume_).half_side_lengths()[0]))) ||
  ...
  protected:
	...
	make_normal();
      ...
      construct_from_points_and_normal();
    }
