...
namespace sl {
  ...
  public:
    ...
    void collapse_edges(mesh_t& M,
      ...
      for (typename mesh_t::const_edge_iterator_t e_it = M.edge_begin();
        ...
        edge_t  e  = e_it->first;
        value_t l2 = len2(M, e);
        if ( l2 < target_edge_length*target_edge_length && is_collapsable(M,e) ) {
  ...
  template <class mesh_t>
  void triangle_mesh_collapse_edges_to_target_edge_length(mesh_t& M, const typename mesh_t::vertex_data_t::value_t target_length) {
    typedef triangle_mesh_edge_collapser<mesh_t> edge_collapser_t;
    edge_collapser_t edge_collapser;
    edge_collapser.collapse_edges(M, target_length, std::size_t(0));
  }
  ...
  template <class mesh_t>
  void triangle_mesh_collapse_boundary_edges_to_target_edge_length(mesh_t& M, const typename mesh_t::vertex_data_t::value_t target_length) {
    typedef triangle_mesh_edge_collapser<mesh_t> edge_collapser_t;
    edge_collapser_t edge_collapser;
    edge_collapser.set_non_boundary_edge_collapse_enabled(false);
    edge_collapser.collapse_edges(M, target_length, std::size_t(0));
  ...
  template <class mesh_t>
  void triangle_mesh_collapse_non_boundary_edges_to_target_edge_length(mesh_t& M, const typename mesh_t::vertex_data_t::value_t target_length) {
    typedef triangle_mesh_edge_collapser<mesh_t> edge_collapser_t;
    edge_collapser_t edge_collapser;
    edge_collapser.set_boundary_edge_collapse_enabled(false);
    edge_collapser.collapse_edges(M, target_length, std::size_t(0));
