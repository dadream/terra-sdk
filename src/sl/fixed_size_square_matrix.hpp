...
namespace sl {
  ...
  public: // Eigenvalues / eigenvectors
    ...
    void symmetric_eigen_in(vector_t& d, self_t& v, bool *ok) const {
      ...
      *ok = false;
      self_t m = (static_cast<const self_t&>(*this));
	...
	d[i] = m(i,i);
      ...
      for (i=0; i<MAX_ITER && !(*ok); i++) {
        // Sum off-diagonale elements
	value_t sm = Zero;
	...
	*ok = (sm == Zero);
	if (!*ok) {
          value_t tresh = (i<3) ? static_cast<value_t>(0.2/sqr(double(dimension)))*sm : Zero;

          for (ip=0; ip<dimension-1; ip++) {
            for (iq =ip+1; iq<dimension; iq++) {
	      // After four sweeps, skip rotation if the off-diagonal element is small
              value_t g = static_cast<value_t>(100)*sl::abs(m(ip,iq));
              if ((i>3) &&
                 (sl::abs(d[ip])+g == sl::abs(d[ip])) &&
                 (sl::abs(d[iq])+g == sl::abs(d[iq]))) {
                m(ip,iq)=Zero;
              } else if (sl::abs(m(ip,iq))>tresh) {
                value_t h =d[iq]-d[ip];
		value_t t;
                if (sl::abs(h)+g == sl::abs(h)) {
                  t = m(ip,iq)/h;
                } else {
                  value_t theta = static_cast<value_t>(0.5)*h/m(ip,iq);
